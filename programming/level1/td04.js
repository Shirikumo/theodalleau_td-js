...
function reply(message) {
    if (message.match(/[Qq]uelle heure est-il/i) !== null) {
    ...
    } else if (message.match(/[Qq]uel est le carré de (\d+)/i)) {
        console.log((message.match(/\d+/)[0])*(message.match(/\d+/)[0]));
        return ((message.match(/\d+/)[0])*(message.match(/\d+/)[0]));
    } else if (message.match(/[Qq]uel âge a une personne née en (\d+)/i)) {
        console.log((2018 - (message.match(/\d+/)[0]) + " ans"));
        return (2018- (message.match(/\d+/)[0]) + " ans");
    }
}
